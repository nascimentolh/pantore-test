import { Cipher, Decipher, createCipheriv, createDecipheriv, scrypt } from "node:crypto";
...
export class CryptoGateway implements EncryptionGateway, DecryptionGateway {
  ...
  public async decrypt(valueEncrypted: string): Promise<string> {
    try {
      ...
      const decryptedValue = Buffer.concat([decipher.update(Buffer.from(valueEncrypted, "hex")), decipher.final()]);
  ...
  private async createDecipher(): Promise<Decipher> {
    ...
    return createDecipheriv("aes-256-ctr", key, Buffer.from(this.environmentConfigService.getEncryptionIV(), "hex"));
  }

  private async createCipher(): Promise<Cipher> {
    const key = await this.getKey();

    return createCipheriv("aes-256-ctr", key, Buffer.from(this.environmentConfigService.getEncryptionIV(), "hex"));
  }

  private async getKey(): Promise<Buffer> {
    return (await promisify(scrypt)(this.environmentConfigService.getEncryptionPassword(), "salt", 32)) as Buffer;
  }
}
